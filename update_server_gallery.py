...
# 1. Update GET route for gallery.php to parse places from gallery.html
get_route_search = r"app\.get\('/pannl/gallery\.php', checkPannlAuth, async \(req, res\) => \{.*?\n    \}\);"
get_route_replace = """app.get('/pannl/gallery.php', checkPannlAuth, async (req, res) => {
  try {
    ...
    // Parse existing places from gallery.html
...
content = re.sub(get_route_search, get_route_replace, content, flags=re.DOTALL)

# 2. Update POST route for add_gallery_place to handle multer image
post_route_search = r"app\.post\('/pannl/add_gallery_place', checkPannlAuth, async \(req, res\) => \{.*?    res\.json\(\{ success: false, message: err\.message \}\);\n  \}\n\}\);"
post_route_replace = """app.post('/pannl/add_gallery_place', checkPannlAuth, upload.single('image'), async (req, res) => {
  ...
  try {
    ...
    let finalImagePath = 'assets/images/portfolio/2.jpg'; // default fallback
...
content = re.sub(post_route_search, post_route_replace, content, flags=re.DOTALL)
