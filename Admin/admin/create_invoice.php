...
<head>
    ...
    <title>Create Invoice - WhiteTake</title>
    ...
    <style>
        ...
        @media (max-width: 768px) {
            ...
            /* Full width for main action buttons only */
            .editor-pane .btn-primary, 
            .editor-pane .btn-secondary {
                width: 100%;
                display: block;
                margin-bottom: 10px;
                color: #000 !important; /* Force visible black text */
            ...
            /* Adjust Add Item / Total Row to stack */
        ...
        .invoice-paper {
            ...
            width: 800px; /* Fixed width for consistent layout before scale */
            min-height: 1000px;
            flex-shrink: 0;
            margin-left: auto;
            margin-right: auto;
        ...
        /* General Styles - Black & White Theme */
        .text-main { color: #000; }
...
<body class="d-flex flex-column">
                                ...
                                <div style="font-size: 0.875rem;">
                                    <table class="ms-auto">
                                        <tr>
                                            ...
                                            <td class="fw-bold text-main text-end"><?php echo date('F d, Y'); ?></td>
                                        </tr>
                                        <tr>
                                            <td class="pe-3 text-muted text-end">Invoice #</td>
                                            <td class="fw-bold text-main text-end">INV-NEW</td>
                                        </tr>
                                    </table>
                                </div>
                            </td>
        ...
        <form method="POST" class="container">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="title" id="titleInput" class="form-control form-control-sm" placeholder="e.g. Wedding Photography" required oninput="updateItems()">
                </div>
                <div class="col-md-4">
            ...
            <div class="table-responsive" style="max-height: 150px; overflow-y:auto;">
                <table class="table table-sm table-bordered mb-2">
                    <tbody id="editorItems">
                        <tr>
                            <td><input type="text" name="items[0][desc]" class="form-control form-control-sm" placeholder="Description" required oninput="updateItems()"></td>
                            <td width="100"><input type="number" step="0.01" name="items[0][price]" class="form-control form-control-sm price-input" placeholder="Price" required oninput="updateItems()"></td>
                            <td width="70"><input type="number" name="items[0][qty]" class="form-control form-control-sm qty-input" value="1" required oninput="updateItems()"></td>
    ...
    <script>
        ...
        function resizePreview() {
            const container = document.getElementById('previewContainer'); // Make sure to add ID to container
            const paper = document.getElementById('invoicePreview');
            if (!container || !paper) return;

            const containerWidth = container.offsetWidth;
            const paperWidth = 800; // Fixed width of our paper
            
            // Only scale if container is smaller than paper (mobile)
            if (containerWidth < paperWidth + 40) {
                const scale = (containerWidth - 20) / paperWidth; 
                paper.style.transform = `scale(${scale})`;
                
                // Adjust height of container to fit scaled paper if needed
        ...
        function addItem() {
            const html = `
                <tr>
                    <td><input type="text" name="items[${itemIndex}][desc]" class="form-control form-control-sm" placeholder="Description" required oninput="updateItems()"></td>
                    <td width="100"><input type="number" step="0.01" name="items[${itemIndex}][price]" class="form-control form-control-sm price-input" placeholder="Price" required oninput="updateItems()"></td>
                    <td width="70"><input type="number" name="items[${itemIndex}][qty]" class="form-control form-control-sm qty-input" value="1" required oninput="updateItems()"></td>
                    <td width="40"><button type="button" class="btn btn-danger btn-sm py-0" onclick="removeRow(this)">&times;</button></td>
                </tr>
            `;
        ...
        function updateItems() {
            ...
            previewTbody.innerHTML = ''; // Clear preview
